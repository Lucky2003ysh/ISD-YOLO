...
class AWLoss(nn.Module):
    def __init__(self, loss_fcn, init_alpha=0.5):
    ...
    def forward(self, pred, true):
        ...
        b1 = (true <= alpha).float()
        a1_1 = (math.exp(1.0 - alpha.item()) - 1) / (math.exp(alpha.item()) - 1)
        a1_2 = 1.0 - a1_1
        a1 = a1_1 * torch.exp(true) + a1_2

        b2 = (true > alpha).float()
        a2 = torch.exp(1.0 - true)

        modulating_weight = a1 * b1 + a2 * b2
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10):  # model must be de-paralleled
        ...
        self.bce = AWLoss(nn.BCEWithLogitsLoss(reduction="none"))
        self.hyp = h
